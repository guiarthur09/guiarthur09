## 👋 Olá! Eu sou o Guilherme Arthur Silveira

Sou estudante de **Desenvolvimento de Sistemas no SENAI**, com 16 anos, sempre buscando evoluir na área de tecnologia e programação.

Tenho experiência com **Python, MySQL, HTML e CSS**, e gosto de desenvolver projetos que envolvem **lógica, automação e banco de dados**, sempre aplicando boas práticas de código.

Atualmente estou focado em aprimorar minhas habilidades técnicas e construir projetos cada vez mais completos e organizados 🚀

---

## 🌐 Linguagens
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

---

## 🛠️ Tecnologias
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

---

## 🚀 Projetos em destaque
- 🟣 **GameChip** — Flask, HTML, CSS, JavaScript e MySQL  
  🔗 https://github.com/TeamGHCP/GameChip

- 👏 **Homenagem ao Professor** — Flask, HTML e CSS  
  🔗 https://github.com/guiarthur09/Projeto-Homengem-Professor

- 🌐 **Sistema de Reservas** — HTML e CSS (projeto desenvolvido no SENAI)  
  🔗 https://github.com/guiarthur09/Sistemaa

---

## 📖 Atualmente aprendendo
- Controle de versão com **Git & GitHub**
- Desenvolvimento Web com **Flask (Python)**
- Boas práticas de **banco de dados (MySQL)**

---

## 🌎 Onde me encontrar
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/_guisilveiraa_)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:guilhermearthursilveira13@gmail.com)

---

## ⚡ Curiosidades
- 🎯 Sempre buscando aprender algo novo
- ☕ Café é combustível para programar
